...
JNIByteCreatable::JNIByteCreatable(
    const std::string & class_factory_classname, const std::string & target_classname, const std::string & binary)
{
    auto * env = JNIHelper::getJNIEnv();

    factory = JNIModuleClassFactory::getFactory(class_factory_classname);
    factory->setQueryId();
