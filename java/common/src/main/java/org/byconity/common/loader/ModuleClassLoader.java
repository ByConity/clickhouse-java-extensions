...
public class ModuleClassLoader extends URLClassLoader {
    ...
    private static final Map<String, ModuleClassLoader> classLoaders = Maps.newHashMap();

    public synchronized static ModuleClassLoader create(String moduleName,
            boolean useByteDependencies) throws MalformedURLException {
        final String jarNameSuffix;
        if (useByteDependencies) {
            jarNameSuffix = String.format("%s-byte-reader-jar-with-dependencies.jar", moduleName);
        } else {
            jarNameSuffix = String.format("%s-reader-jar-with-dependencies.jar", moduleName);
        }
        if (classLoaders.containsKey(jarNameSuffix)) {
            return classLoaders.get(jarNameSuffix);
        }
        ModuleClassLoader moduleClassLoader = getModuleClassLoader(jarNameSuffix);
        classLoaders.put(jarNameSuffix, moduleClassLoader);
        return moduleClassLoader;
    }

    private static ModuleClassLoader getModuleClassLoader(String jarNameSuffix)
