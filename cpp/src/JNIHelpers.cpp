...
namespace jni
{
    ...
    jthrowable
    findMethodId(JNIEnv * env, jmethodID * out, jclass cls, const char * method_name, const char * method_signature, bool static_method)
    {
        jmethodID mid = nullptr;
        if (static_method)
            mid = env->GetStaticMethodID(cls, method_name, method_signature);
        else
    ...
    // NOLINTNEXTLINE
    jthrowable invokeObjectMethod(JNIEnv * env, jvalue * retval, jobject instance_obj, jmethodID mid, const char * method_signature, ...)
    {
        va_list args;
        jthrowable jthr;
        const char * str;
        ...
        if (return_type == JOBJECT || return_type == JARRAYOBJECT)
        {
            ...
            jobj = env->CallObjectMethodV(instance_obj, mid, args);
            retval->l = jobj;
        }
        else if (return_type == JVOID)
        {
            env->CallVoidMethodV(instance_obj, mid, args);
        }
        else if (return_type == JBOOLEAN)
        {
            jboolean jbool = 0;
            jbool = env->CallBooleanMethodV(instance_obj, mid, args);
            retval->z = jbool;
        }
        else if (return_type == JSHORT)
        {
            jshort js = 0;
            js = env->CallShortMethodV(instance_obj, mid, args);
            retval->s = js;
        }
        else if (return_type == JLONG)
        {
            jlong jl = -1;
            jl = env->CallLongMethodV(instance_obj, mid, args);
            retval->j = jl;
        }
        else if (return_type == JINT)
        {
            jint ji = -1;
            ji = env->CallIntMethodV(instance_obj, mid, args);
            retval->i = ji;
        }
        va_end(args);
    ...
    // NOLINTNEXTLINE
    jthrowable invokeStaticMethod(JNIEnv * env, jvalue * retval, jclass cls, jmethodID mid, const char * method_signature, ...)
    {
        va_list args;
        jthrowable jthr;
        const char * str;
    ...
    // NOLINTNEXTLINE
    jthrowable invokeNewObject(JNIEnv * env, jobject * instance_obj, jclass cls, jmethodID mid, ...)
    {
        va_list args;
        jthrowable jthr;

        va_start(args, mid);
        *instance_obj = env->NewObjectV(cls, mid, args);
        va_end(args);
    ...
    jobject mapToJmap(JNIEnv * env, const std::map<std::string, std::string> & params)
    {
        ...
        jobject jmap;
        THROW_JNI_EXCEPTION(env, invokeNewObject(env, &jmap, jcls_hashmap, mid_ctor));
