/*
 * Copy From Impala repo with slight modifications.
 */
...
/**
 * Manages a pool of RetryingMetaStoreClient connections. If the connection pool is empty a new
 * client is created and added to the pool. The idle pool can expand till a maximum size of
 * MAX_HMS_CONNECTION_POOL_SIZE, beyond which the connections are closed.
 *
 * This default implementation reads the Hive metastore configuration from the HiveConf object
 * passed in the c'tor. If you are looking for a temporary HMS instance created from scratch for
 * unit tests, refer to EmbeddedMetastoreClientPool class. It mocks an actual HMS by creating a
 * temporary Derby backend database on the fly. It should not be used for production Catalog server
 * instances.
 */
...
public class MetaStoreClientPool {
    ...
    private static final String HIVE_METASTORE_CNXN_DELAY_MS_CONF =
            "lf.metastore.cnxn.creation.delay.ms";
    ...
    /**
     * A wrapper around the RetryingMetaStoreClient that manages interactions with the connection
     * pool. This implements the AutoCloseable interface and hence the callers should use the
     * try-with-resources statement while creating an instance.
     */
    public class MetaStoreClient implements AutoCloseable {
        ...
        /**
         * Creates a new instance of MetaStoreClient. 'cnxnTimeoutSec' specifies the time
         * MetaStoreClient will wait to establish first connection to the HMS before giving up and
         * failing out with an exception.
         */
        private MetaStoreClient(Map<String, String> params, HiveConf hiveConf, int cnxnTimeoutSec) {
            ...
            while (true) {
                try {
                    ...
                    hiveClient.setService("catalog_service");
                    hiveClient.setRegion(params.getOrDefault("region", "cn-beijing"));
                    hiveClient.setAccessKeyIdAndSecretAccessKey(getOrThrow(params, "access_key"),
        ...
        /**
         * Returns this client back to the connection pool. If the connection pool has been closed,
         * just close the Hive client connection.
         */
        @Override
        public void close() {
    ...
    /**
     * Initialize client pool with 'numClients' client. 'initialCnxnTimeoutSec' specifies the time
     * (in seconds) the first client will wait to establish an initial connection to the HMS.
     */
    public void initClients(int numClients, int initialCnxnTimeoutSec) {
        Preconditions.checkState(clientPool_.size() == 0);
        if (numClients > 0) {
            clientPool_.add(new MetaStoreClient(params_, hiveConf_, initialCnxnTimeoutSec));
            for (int i = 0; i < numClients - 1; ++i) {
                clientPool_.add(new MetaStoreClient(params_, hiveConf_, 0));
            }
        }
    }
    ...
    public MetaStoreClient getClient() {
        ...
        if (client == null) {
            synchronized (this) {
                ...
                client = new MetaStoreClient(params_, hiveConf_, 0);
            }
        }
        client.markInUse();
        return client;
    ...
    /**
     * Removes all items from the connection pool and closes all Hive Meta Store client connections.
    ...
    public int getNumHmsClientsIdle() {
        return clientPool_.size();
    }

    public int getNumHmsClientsInUse() {
        return numHmsClientsInUse_.get();
    }
}
