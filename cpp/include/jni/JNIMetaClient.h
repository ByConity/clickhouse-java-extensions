...
public:
    ...
    std::string getIcebergSchema(const std::map<std::string, std::string> & params);
    // IcebergScanInfo represents a tuple of (data_files)
    using IcebergScanInfo = std::tuple<std::vector<std::string>>;
    IcebergScanInfo getIcebergScanInfo(const std::map<std::string, std::string> & params);
