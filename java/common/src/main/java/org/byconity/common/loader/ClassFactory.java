...
public abstract class ClassFactory {
    ...
    protected ClassFactory(Map<String, String> options) {
        try {
            // For paimon and iceberg, dependencies of hadoop are different for internal hdfs and
            // community hdfs,
            // so we need to distinguish them.
            final boolean useByteDependencies;
            if (Objects.isNull(options)) {
                useByteDependencies = false;
            } else {
                useByteDependencies =
                        Boolean.parseBoolean(options.getOrDefault("useByteDependencies", "false"));
            }
            classLoader = ModuleClassLoader.create(getModuleName(), useByteDependencies);
        } catch (Exception e) {
